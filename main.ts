...
const icons: Record<string, string> = {
  ...
  attach:
    '<path d="m8 12 6-6a4 4 0 0 1 6 6l-9 9a6 6 0 0 1-8-8l9-9m-6 10 8-8a2 2 0 0 1 3 3l-8 8a2 2 0 0 1-3-3"/>',
  terminal:
  ...
  clipboard:
    '<rect x="5" y="5" width="14" height="16" rx="2"/><rect x="9" y="3" width="6" height="4" rx="1"/>',
...
let client: ExtensionClient,
  history: History,
  conversation = fresh(),
  record: AppValue | null = null;
let connection: AppConnection | null = null,
...
function changed() {
  dirty = true;
  conversation.updated = Math.max(Date.now(), conversation.updated + 1);
  updateDocument();
...
function save() {
  ...
  const task = saves.then(async () => {
    ...
    if (conversation.updated === timestamp) {
      dirty = false;
      updateDocument();
    }
    await renderHistory();
...
async function renderHistory() {
  ...
  if (!rows.length)
    list.append(
      el("p", "history-empty", "Your next conversation starts here."),
    );
  for (const row of rows) {
    const button = el(
      "button",
      `history-item${row.id === conversation.id ? " selected" : ""}`,
    );
    button.append(
      el("span", "", row.title),
      el(
    ...
    button.onclick = () => {
      void switchConversation(row.id).catch(report);
    };
    list.append(button);
  }
}
  ...
  segments.forEach((segment, index) => {
    if (index % 2) {
      ...
      const block = el("div", "code-block"),
        top = el("div", "code-header", language || "code"),
        copy = el("button", "icon-button");
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const paragraph = el(
        heading ? "h3" : "p",
        /^\s*[-*]\s/.test(line) ? "text-bullet" : "",
      );
      const parts = (heading ? heading[2] : line).split(
        /(\*\*[^*]+\*\*|`[^`]+`)/g,
      );
      for (const part of parts)
        paragraph.append(
          part.startsWith("**") && part.endsWith("**")
            ...
            : part.startsWith("`") && part.endsWith("`")
              ? el("code", "inline-code", part.slice(1, -1))
...
function messageCard(role: "user" | "assistant", content: string) {
  const article = el("article", `message ${role}`),
    ...
    body = el("div", "message-main");
  avatar.innerHTML = role === "assistant" ? icon("spark") : "Y";
  body.append(
    el("div", "message-author", role === "assistant" ? "Canvas" : "You"),
  );
      ...
      if (Array.isArray(message.content))
        for (const part of message.content) {
          if (
            ...
            /^data:image\/(png|jpeg|webp);base64,/.test(part.image_url.url)
          ) {
...
function renderAttachments() {
  ...
  for (const attachment of conversation.attachments) {
    ...
    chip.append(el("span", "attachment-name", attachment.name));
    const remove = el("button", "icon-button");
    remove.innerHTML = icon("close");
    remove.setAttribute("aria-label", `Remove ${attachment.name}`);
    remove.disabled = busy;
    remove.onclick = () => {
      conversation.attachments = conversation.attachments.filter(
...
function addAttachment(attachment: Omit<Attachment, "id">) {
  if (busy)
    throw new Error("Wait for the current response before adding context.");
  if (
    conversation.attachments.length >= 8 ||
    conversation.attachments.reduce(
    ...
    throw new Error(
      "Attach up to eight items totaling 1.8 million characters per message. Use a smaller image or select a text excerpt.",
    );
  conversation.attachments.push({ ...attachment, id: crypto.randomUUID() });
  renderAttachments();
  changed();
}
async function imageAttachment(blob: Blob, name: string) {
  ...
  try {
    ...
    addAttachment({
      name,
      kind: "image",
      content: canvas.toDataURL("image/jpeg", 0.84),
    });
  } finally {
    /* The source is a user-selected data URL allowed by the isolated app CSP. */
  }
}
const review: ReviewAction = (action, signal) =>
  new Promise((resolve) => {
    if (signal.aborted) {
      resolve(false);
      return;
    ...
    panel.hidden = false;
    panel.querySelector("h3")!.textContent = action.title;
    panel.querySelector(".approval-target")!.textContent =
      `Target: ${action.target}`;
    panel.querySelector("pre")!.textContent = action.detail;
    ...
    pendingReview = finish;
    signal.addEventListener("abort", abort, { once: true });
    $("deny-action").onclick = () => finish(false);
    $("approve-action").onclick = () => finish(true);
    $("deny-action").focus();
  });
function setBusy(value: boolean) {
  ...
  $("send").innerHTML = icon(value ? "stop" : "arrow");
  $("send").setAttribute(
    "aria-label",
    value ? "Stop response" : "Send message",
  );
  $("send").title = value ? "Stop response" : "Send message";
...
async function send(continuing = false) {
  ...
  try {
    ...
    if (new TextEncoder().encode(JSON.stringify(messages)).length > 2600000)
      throw new Error(
    ...
    const sync = () => {
      conversation.messages.push(...messages.slice(persisted));
    ...
    try {
      await runAgent({
        ...
        onText(text) {
          ...
          $("activity").textContent = "Writing…";
          if (follow) scroller.scrollTop = scroller.scrollHeight;
        },
        onTool(call, state, result) {
          sync();
          toolCard(call, state, result);
      ...
      if (
        currentAssistantText &&
        messages.at(-1)?.content !== currentAssistantText
      )
        conversation.partial = currentAssistantText;
      throw error;
    }
  } catch (error) {
    ...
    setBusy(false);
    renderThread();
    await save().catch(report);
    prompt.focus();
  }
}
function showSettings(show: boolean) {
  $("settings-panel").hidden = !show;
  root.classList.toggle("settings-open", show);
  if (show) {
...
function updateConnection() {
  $("model-label").textContent = model || "Connect a model";
...
async function refreshEnvironment() {
  ...
  $("host-caption").textContent =
    environment.connection === "connected"
      ? environment.host?.system || "Connected through ShellCanvas"
      : environment.connection === "review-required"
        ? "Accept the new connection in the window bar"
        : environment.connection === "disconnected"
          ? "Disconnected · conversations stay available"
          : "Connect a host to use workspace tools";
  const methods = await client.services.list();
  const available = methods.filter(
    (method) => method.available && method.granted,
  );
  $("tools-label").textContent = toolsEnabled
    ? environment.connection === "connected"
      ? "Workspace tools ready · changes require review"
      : "Chat is ready · no remote tools connected"
    : "Chat only · workspace tools off";
  const allowed = (name: string) =>
    available.some((method) => method.name === name);
  $<HTMLButtonElement>("attach-remote").disabled =
    busy ||
    !allowed("system.files.readText") ||
    !allowed("system.dialogs.openFile");
  $<HTMLButtonElement>("paste-attachment").disabled =
    busy ||
    (!allowed("system.clipboard.readStart") &&
      !allowed("system.clipboard.image.readStart"));
  if (
    active &&
    environment.connection !== "connected" &&
    conversation.host !== "Local workspace"
  )
    active.abort(new Error("Connection changed."));
}
async function start() {
  ...
  if (
    !access.some(
      (method) =>
        method.name === "system.storage.put" &&
        method.available &&
        method.granted,
    )
  ) {
    client = { ...client, storage: memoryStorage(), settings: memoryStorage() };
    const privacy = root.querySelector(".sidebar-bottom > div")!;
    privacy.textContent = "Session-only history";
    privacy.append(el("small", "", "Storage permission is not granted"));
    notify(
      "History is available for this window only because storage permission was not granted.",
    );
  }
  ...
  const unsubscribe = client.events.subscribe(() => {
    void refreshEnvironment().catch(report);
  }, report);
  ...
  $("new-chat").onclick = () => {
    void switchConversation().catch(report);
  };
  ...
  for (const id of ["settings", "model-button"])
    $(id).onclick = () => showSettings($("settings-panel").hidden);
  $("close-settings").onclick = () => showSettings(false);
  $("configure-model").onclick = () => {
    void client.network
      .configure({
        slot: "model",
        suggestedEndpoint:
          connection?.endpoint ?? "https://api.openai.com/v1/responses",
      })
      .then((value) => {
  ...
  $("forget-model").onclick = () => {
    void client.network
      .forget("model")
      .then(() => {
        connection = null;
        updateConnection();
        notify("Connection forgotten.");
      })
      .catch(report);
  };
  $("save-settings").onclick = () => {
    void (async () => {
      ...
      await refreshEnvironment();
      showSettings(false);
      notify("Assistant settings saved.");
    })().catch(report);
  };
  $("export-chat").textContent = "Copy conversation JSON";
  $("export-chat").onclick = () => {
    void client.clipboard
      .writeText(JSON.stringify(conversation, null, 2))
      .then(() =>
        notify(
          "Conversation JSON copied. Paste it into a local file to keep an export.",
        ),
      )
      .catch(report);
  };
  $("duplicate-chat").onclick = () => {
    void (async () => {
      await saves;
      conversation = {
      ...
      changed();
      await save();
      notify("Saved as a separate conversation.");
    })().catch(report);
  };
  $("delete-chat").onclick = () => {
    void (async () => {
      const result = await client.system.dialogs.messageBox({
        title: "Delete conversation?",
        message: `Delete “${conversation.title}” and its local attachments?`,
        kind: "warning",
        buttons: [
          { id: "cancel", label: "Keep conversation" },
  ...
  $<HTMLInputElement>("local-files").onchange = (event) => {
    const files = Array.from((event.target as HTMLInputElement).files ?? []);
    (event.target as HTMLInputElement).value = "";
    void (async () => {
      for (const file of files) {
        if (file.type.startsWith("image/"))
          await imageAttachment(file, file.name);
        else {
          if (file.size > 300000)
            throw new Error(
              "Choose a text file smaller than 300 KB, or paste an excerpt.",
            );
          ...
          if (text.includes("\0"))
            throw new Error(
              "This appears to be a binary file. Attach a text file or image.",
            );
          addAttachment({ name: file.name, kind: "text", content: text });
        }
      }
    })().catch(report);
  };
  $("attach-remote").onclick = () => {
    void (async () => {
      const accepted = await client.environment.get();
      if (!accepted.binding) throw new Error("Connect a host first.");
      const files = await client.system.dialogs.openFile({
        title: "Attach remote text",
        multiple: true,
      });
      for (const file of files ?? []) {
        ...
        addAttachment({ name: doc.name, kind: "text", content: doc.text });
      }
    })().catch(report);
  };
  $("paste-attachment").onclick = () => {
    void (async () => {
      let image;
      try {
        image = await client.clipboard.readImage();
      } catch {
        /* A text-only clipboard is ordinary. */
      }
      if (image) {
        const canvas = document.createElement("canvas");
        ...
        const blob = await new Promise<Blob>((resolve, reject) =>
          canvas.toBlob(
            (blob) =>
              blob
                ? resolve(blob)
                : reject(new Error("Cannot encode clipboard image.")),
            "image/png",
          ),
        );
        await imageAttachment(blob, "Clipboard image");
      } else {
        const text = await client.clipboard.readText();
        if (!text) throw new Error("No text or image on the clipboard.");
        addAttachment({ name: "Clipboard text", kind: "text", content: text });
      }
    })().catch(report);
  };
...
void start().catch((error) => {
  report(error);
  $("tools-label").textContent = "Desktop services unavailable";
});
